...
//
                var hcluster = function () {
                    ...
                    clust.orderedNodes = function () {
                        if (!treeRoot)
                            throw new Error('Need to passin data and build tree first.');
                    ...
                    clust.getClusters = function (n) {
                        if (!treeRoot)
                            throw new Error('Need to passin data and build tree first.');
                        if (n > data.length)
                            throw new Error('n must be less than the size of the dataset');
                        return clustersGivenK[data.length - n]
                            .map(function (indexes) {
                                return indexes.map(function (ndx) {
                                    return data[ndx];
                                });
                            });
                    };
                    ...
                    clust._buildTree = function () {
                        if (!data || !data.length)
                            throw new Error('Need `data` to build tree');
                        ...
                        for (var iter = 0; iter < data.length - 1; iter++) {
                            ...
                            nearestPair = clusterPairs
                                .reduce(function (pairA, pairB) {
                                    return pairA[2] <= pairB[2] ? pairA : pairB;
                                },
                                    [0, 0, Infinity]);
                            newCluster = {
                                name: 'Node ' + iter,
                            ...
                            clustersGivenK.push(clusters.map(function (c) {
                                return c.indexes;
                            }));
